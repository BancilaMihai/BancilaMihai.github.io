...
// asa se numeste functia += --- left hand side de tip t_set_of_primes ---right hand side de tip Int
...
func +(lhs: t_set_of_primes, rhs: t_set_of_primes)->t_set_of_primes{
    //var tmp = t_set_of_primes() asta daca nu aveam let
    //reunim lhs.reuneste2 cu rhs si il punem in tmp
...
var test = teste.sterge
    ...
    switch test.rawValue{ //row = brut
        ...
        case "7": // testam += <<
            let m = t_set_of_primes(7)
            m += 2 // supraincarcam operatorul +=
            m << 3 // supraincarcam operatorul <<
            m << 4
            m.print_set()
        case "8": // testam reuniune 3
            let m = t_set_of_primes(7, 41)
            let x = t_set_of_primes(90, 100)
            var y = t_set_of_primes(100, 200)
            y = m + x
        ...
        default: //pentru ca putem acoperi toate cazurile avem nevoie de acest default
            print("Invalid input")
    }